// Sesión firmada por HMAC, guardada en una cookie httpOnly. Sin dependencias
// nuevas: crypto es nativo de Node (adapter-node), y timingSafeEqual evita
// que comparar la firma carácter por carácter filtre SESSION_SECRET por timing.
import { createHmac, timingSafeEqual } from 'node:crypto';
import { env } from '$env/dynamic/private';

export type SesionUsuario = { id: number; nombre: string; tokenVersion: number };

export const SESSION_COOKIE = 'fortunecity_sesion';
...
export function verificarSesion(valor: string): SesionUsuario | null {
